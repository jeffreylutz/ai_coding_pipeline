...
class TestKiroInstallation:
    ...
    @given(
        config_overrides=st.dictionaries(
            keys=st.sampled_from(["sandbox.enabled", "autonomous_operation.enabled", "workspace"]),
            values=st.one_of(
                st.booleans(),
                st.text(min_size=1, max_size=20).filter(lambda x: "/" in x or x.isalnum())
            ),
            min_size=1,
            max_size=3
        )
    )
    def test_kiro_configuration_flexibility_property(self, config_overrides: Dict[str, Any]):
        """
        Property test: For any valid configuration overrides, Kiro should handle
        ...
        container = self.client.containers.run(
            self.image_name,
            name=f"{self.container_name}-config-flex",
            detach=True,
            tty=True,
            remove=True
        )
        ...
        try:
            ...
            exit_code, output = container.exec_run("cat /opt/configs/kiro/config.json")
            assert exit_code == 0, "Should be able to read original configuration"
            
            original_config = json.loads(output.decode())
            ...
            for key, value in config_overrides.items():
                if "." in key:
                    # Handle nested keys
                    parts = key.split(".")
                    if len(parts) == 2 and parts[0] in modified_config:
                        if isinstance(modified_config[parts[0]], dict):
                            modified_config[parts[0]][parts[1]] = value
                else:
            ...
            # Validate that the modified configuration is still valid JSON
            exit_code, output = container.exec_run("python -m json.tool /tmp/kiro_config_test.json")
